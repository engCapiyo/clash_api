...
use crate::{
    ...
    models::archive::{
        ActivityBreakdown, ActivityType, AppliedFilters, ArchiveActivity, ArchiveActivityRequest,
        ArchiveActivityResponse, ArchiveQueryParams, ArchiveStats, GetAllArchiveQuery,
        PaginatedArchiveResponse, PaginationInfo, UserArchiveStats,
    },
    state::AppState,
};
...
pub async fn get_user_archive(
    ...
    println!(
        "✅ Found {} archive activities for user: {}",
        activities.len(),
        user_id
    );
    Ok(Json(activities))
...
pub async fn get_fixture_archive(
    ...
    println!(
        "✅ Found {} archive activities for fixture: {}",
        activities.len(),
        fixture_id
    );
    Ok(Json(activities))
...
pub async fn get_user_archive_stats(
    ...
    println!(
        "✅ Stats for user {}: {} votes, {} likes, {} comments",
...
pub async fn delete_archive_activity(
    ...
    let object_id =
        ObjectId::parse_str(&id).map_err(|_| AppError::invalid_data("Invalid activity ID"))?;
...
pub async fn get_recent_archive_activities(
    ...
    println!(
        "✅ Found {} recent archive activities for user: {}",
        activities.len(),
        user_id
    );
    Ok(Json(activities))
...
pub async fn check_user_activity(
    ...
    println!(
        "🔍 Checking user activity for {} on fixture {}",
        user_id, fixture_id
    );
